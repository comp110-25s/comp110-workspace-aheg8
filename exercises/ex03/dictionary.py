...
def invert(d: dict[str, str]) -> dict[str, str]:
    """Inverts dictionary keys to equal their respective values."""
    keys = list(d.keys())
    values = list(d.values())

    if len(set(values)) != len(values):
        raise KeyError("Duplicate value, cannot invert.")
...
def count(counter: list[str]) -> dict[str, int]:
    """Counts the frequencies of values and returns totals in a dictionary."""
    result: dict[str, int] = {}
    idx: int = 0
...
def favorite_color(favorites: dict[str, str]) -> str:
    """Establishes which color is the favorite by utilizing count fn."""
...
def bin_len(input: list[str]) -> dict[int, set[str]]:
    """Returns dict of int of length of input word."""
